...
import com.example.architecturepractice.customview.adapter.MonthFragmentStateAdapter
...
class CalendarFragment : DaggerFragment() {
    ...
    private fun getViewBinding(): FragmentCalendarBinding =
    ...
    private fun initViews() {
        ...
        if(autoLogin == true) {
            fetchJob = viewModel.fetchLocalData()
        } else {
    ...
    private fun initViewPager() = with(binding) {
        ...
        monthViewPager.offscreenPageLimit = 5
    ...
    private fun setCheckBox(list: List<CalendarEntity>) = with(binding) {
        ...
        calendarNavigationView.menu.add("내 캘린더")
